...
// wipe the fields
	...
	if (which != selectedTexture)
	{
        ...
        [textureView_i scrollRectToVisibleAsNXRect:&r];//S&F*****[textureView_i scrollRectToVisible:&r];
		[textureView_i display];
		sprintf(string,"%d x %d",(int)t->r.size.width,
			(int)t->r.size.height - TEX_SPACING);
		[sizeField_i /*/S&F*****setStringValue*/setStringValueAsString:string];
	}
...
- searchForTexture:sender
{
	...
	max = [textureList_i count];
	strcpy(name,(const char *)[sender /*//S&F*****stringValue*/stringValueAsString]);
    [sender /*/S&F*****setStringValue*/setStringValueAsString:strupr(name)];
	len = strlen(name);
