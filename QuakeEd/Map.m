...
- addSelected
{
	...
	w = [list objectAtIndex: 0];	// world object
...
- removeObject: (id)o
{
	...
	if (o == currentEntity)
	{	// select the world
		[self setCurrentEntity: [list objectAtIndex: 0]];
	}
...
- writeMapFile: (char *)fname useRegion: (BOOL)reg
{
	...
	for (i=0 ; i<list.count ; i++)
		[[list objectAtIndex: i] writeToFILE: f region: reg];
...
- ZDrawSelf
{
	...
	for (i=0 ; i<count ; i++)
		[[list objectAtIndex: i] ZDrawSelf];
...
- RenderSelf: (void (*) (face_t *))callback
{
	...
	for (i=0 ; i<count ; i++)
		[[list objectAtIndex: i] RenderSelf: callback];
...
- entityConnect: (vec3_t)p1 : (vec3_t)p2
{
	...
	if (oldent == [list objectAtIndex: 0])
	{
...
- selectRay: (vec3_t)p1 : (vec3_t)p2 : (BOOL)ef
{
	...
	if ( ![bestbrush selected] )
	{
		...
		qprintf ("selected entity %i brush %i face %i", [list indexOfObject:bestent], [bestent indexOfObject: bestbrush], bestface);
	}
	else 
	{
...
- setTextureRay: (vec3_t)p1 : (vec3_t)p2 : (BOOL)allsides;
{
	...
	if (allsides)
	{
		...
		qprintf ("textured entity %i brush %i", [list indexOfObject:bestent], [bestent indexOfObject: bestbrush]);
	}
	else 
	{
...
// copy entire entities otherwise
	originalElements = list.count;	// don't copy the new ones
	for (i=1 ; i<originalElements ; i++)
	{
		o = [list objectAtIndex: i];
