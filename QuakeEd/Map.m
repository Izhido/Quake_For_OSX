...
/*S&F*****- RenderSelf: (void (*) (face_t *))callback
{
	int		i, count;
	...
	for (i=c-1 ; i>=0 ; i--)
	{
		...
		for (j = c2-1 ; j >=0 ; j--)
		{
			...
			if ([brush regioned])
				continue;
			total++;
			[brush /*/S&F*****perform*/performSelector:sel];
		}
	}
...
// copy individual brushes in the world entity
	...
	// copy the brush, then translate the original
		new = [b copy];
		[/*/S&F*****/(SetBrush*)/*/S&F*****/new setSelected: YES];
		[new translate];
		[/*/S&F*****/(SetBrush*)/*/S&F*****/b setSelected: NO];
		[o addObject: new];
	}
...
// copy entire entities otherwise
	...
	for (i=1 ; i<originalElements ; i++)
	{
		...
		c = [o count];
		for (j=0 ; j<c ; j++)
			[/*/S&F*****/(SetBrush*)/*/S&F*****/[o objectAt: j] setSelected: NO];
		...
		for (j=0 ; j<c ; j++)
		{
			b = [new objectAt: j];
			[b translate];
			[/*/S&F*****/(SetBrush*)/*/S&F*****/b setSelected: YES];
		}
	}
...
- selectCompleteEntity: sender
{
	...
	c = [o count];
	for (i=0 ; i<c ; i++)
		[/*/S&F*****/(SetBrush*)/*/S&F*****/[o objectAt: i] setSelected: YES];	
...
- makeEntity: sender
{
	...
	{	// throw out seed brush and select entity fixed brush
		...
		[/*/S&F*****/(SetBrush*)/*/S&F*****/[sb_newowner objectAt: 0] setSelected: YES];
	}
