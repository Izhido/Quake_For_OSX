...
- setProjectPath:(char *)path
{
    ...
    [startproject_i /*/S&F*****setStringValue*/setStringValueAsString: path];
...
{
	[startproject_i /*/S&F*****setStringValue*/setStringValueAsString: [project_i currentProjectFile]];
	[self UIChanged: self];
	return self;
}
...
- setBspSound:sender
{
	...
	rtn = [panel
			/*/S&F*****runModalForDirectory*/runModalForDirectoryAsString:path
			file: file
			types: types];

	if (rtn)
	{
		filename = (char **)[panel /*/S&F*****filenames*/filenamesAsStringList];
		strcpy(bspSound,[panel /*/S&F*****directory*/directoryAsString]);
		strcat(bspSound,"/");
...
- setBspSoundPath:(char *)path
{
	...
	if (!bspSound_i)
	{
        strcpy (bspSound, "/System/Library/Sounds/Funk.aiff");//*/S&F*****"/NextLibrary/Sounds/Funk.snd");
		bspSound_i = [[Sound alloc] initFromSoundfile:bspSound];
	}

	[bspSoundField_i /*/S&F*****setStringValue*/setStringValueAsString:bspSound];
...
-UIChanged: sender
{
	...
	[self setProjectPath: (char *)[startproject_i /*//S&F*****stringValue*/stringValueAsString]];
	[self setBspSoundPath: (char *)[bspSoundField_i /*//S&F*****stringValue*/stringValueAsString]];
