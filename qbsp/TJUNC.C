...
restart:	
	...
	// past lastcorner
		...
		/*/S&F*****new*/newface = NewFaceFromFace (f);
		if (f->original)
			Error ("SplitFaceForTjunc: f->original");
			
		/*/S&F*****new*/newface->original = chain;
		chain = /*/S&F*****new*/newface;
		/*/S&F*****new*/newface->next = newlist;
		newlist = /*/S&F*****new*/newface;
		if (f->numpoints - firstcorner <= MAXPOINTS)
			/*/S&F*****new*/newface->numpoints = firstcorner+2;
		else if (lastcorner+2 < MAXPOINTS &&
		f->numpoints - lastcorner <= MAXPOINTS)
			/*/S&F*****new*/newface->numpoints = lastcorner+2;
		else
			/*/S&F*****new*/newface->numpoints = MAXPOINTS;

		for (i=0 ; i</*/S&F*****new*/newface->numpoints ; i++)
		{
			VectorCopy (f->pts[i], /*/S&F*****new*/newface->pts[i]);
		}
		
		
		for (i=/*/S&F*****new*/newface->numpoints-1 ; i<f->numpoints ; i++)
		{
			VectorCopy (f->pts[i], f->pts[i-(/*/S&F*****new*/newface->numpoints-2)]);
		}
		f->numpoints -= (/*/S&F*****new*/newface->numpoints-2);
	} while (1);
